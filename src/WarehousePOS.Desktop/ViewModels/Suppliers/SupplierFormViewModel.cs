...
public sealed class SupplierFormViewModel : ViewModelBase
{
    ...
    public SupplierFormViewModel(ISupplierService service)
    {
        _service = service;
    ...
    public void Load(SupplierDto? dto = null)
    {
        _editingId     = dto?.Id;
        ...
        try
        {
            if (_editingId.HasValue)
                await _service.UpdateAsync(new UpdateSupplierRequest(
                    _editingId.Value, Name, Null(ContactPerson), Null(Phone), Null(Email), Null(Address)));
            else
                await _service.CreateAsync(new CreateSupplierRequest(
                    Name, Null(ContactPerson), Null(Phone), Null(Email), Null(Address)));
